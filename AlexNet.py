...
def AlexNet_model_with_additional_data(
    input_shape, additional_info_dim, n_class, learning_rate
) -> Sequential:
    image_input = Input(shape=input_shape)
    x = Conv2D(filters=128, kernel_size=(11, 11), strides=(4, 4), activation="relu")(
        image_input
    )
    ...
    combined = concatenate(
        [
            image_model.output,
            additional_model.output,
            additional_model_2.output,
            additional_model_3.output,
        ]
    )
    ...
    model = Model(
        inputs=[
            image_model.input,
            additional_model.input,
            additional_model_2.input,
            additional_model_3.input,
        ],
        outputs=z,
    )
